...
src2 <- ga$getData(
  ...
  metrics = "ga:sessions",
  batch = TRUE,
  # segment = "users::sequence::^ga:userType==New%20Visitor;dateOfSession<>2017-11-01_2017-11-07;ga:campaign==Campaign%20A;->>perSession::ga:transactions>0",
  # max.results = 100,
  sort = "ga:date"
)
...
plot(analytics$adClicks) # Number of visits
plot(analytics$adCost)   # Cost 

write.csv(analytics, file = "ga1.csv", row.names = FALSE)
data <- read.csv(url("https://raw.githubusercontent.com/kruny1001/tsProject/master/ga1.csv"))

plot(data$users)
plot(data$adClicks)
plot(data$adCost)
plot(data$goalConversionRateAll)

completeData <- data %>% filter(users > 0)
nrow(completeData)
plot.ts(completeData$users)
plot.ts(completeData$adClicks)
plot.ts(completeData$adCost)
plot.ts(completeData$goalConversionRateAll)
```
